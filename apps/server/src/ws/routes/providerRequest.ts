...
import z from 'zod'
import { sendRequestAndWaitForResponse } from '../../utils'

// const bodySchema = z.record(z.string(), z.number())
const bodySchema = z.object({
    request: z.enum(['importBlocks', 'exportBlocks', 'checkStorage']),
    blocks: z
        .union([z.record(z.string(), z.number()), z.array(z.never())])
...
export async function providerRequestHandler({
    ...
    const { request, blocks } = bodySchema.parse(body)
    ...
    const response = await sendRequestAndWaitForResponse(
        providerSocket,
        request,
        blocks
    )
    sendResponse(responseSchema, response)
